...
export class LocataireComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.locataireService
      ...
      .subscribe(
        (res: HttpResponse<ILocataire[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  ngOnInit(): void {
    ...
    this.registerChangeInLocataires();
  }
  ...
  trackId(index: number, item: ILocataire): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInLocataires(): void {
    this.eventSubscriber = this.eventManager.subscribe('locataireListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ILocataire[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
