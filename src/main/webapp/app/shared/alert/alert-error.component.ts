...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(private alertService: JhiAlertService, private eventManager: JhiEventManager, translateService: TranslateService) {
    this.errorListener = eventManager.subscribe('gestLocativeApp.error', (response: JhiEventWithContent<AlertError>) => {
      const errorResponse = response.content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = eventManager.subscribe('gestLocativeApp.httpError', (response: JhiEventWithContent<HttpErrorResponse>) => {
        ...
        case 400: {
          ...
          if (errorHeader) {
            ...
            for (const fieldError of fieldErrors) {
              ...
              const fieldName = translateService.instant('gestLocativeApp.' + fieldError.objectName + '.' + convertedField);
